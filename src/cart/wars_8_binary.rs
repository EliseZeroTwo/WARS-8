...
impl Wars8Binary {
    ...
    pub fn new(path: &String) -> Wars8Binary {
        ...
        for sprite in 0..sprite_count {
            ...
            for row in 0..8 {
                for col in 0..8 {
                    sprites[sprite as usize][row][col] = ColorPallete::from(sprite_buffer[(row * 8) + col] as i32);
                }
            }
        }
...
impl Cart for Wars8Binary {
    ...
    fn save(&self) -> Result<(), ()> {
        if self.binary().len() > u32::MAX as usize {
            println!("Binary too large! {} bytes over limit, still going to save...", self.binary().len() - u32::MAX as usize);
        }
